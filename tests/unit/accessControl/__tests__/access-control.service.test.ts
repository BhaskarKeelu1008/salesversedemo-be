...
import type { ModuleConfigDto } from '@/modules/accessControl/dto/create-access-control.dto';
...
describe('AccessControlService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    mockAccessControlRepository = new AccessControlRepository() as jest.Mocked<AccessControlRepository>;
    accessControlService = new AccessControlService();
    (accessControlService as any).accessControlRepository = mockAccessControlRepository;
  });
  ...
  describe('getOrCreateDefaultAccessControl', () => {
    ...
    beforeEach(() => {
      ...
      (ModuleModel.findById as jest.Mock).mockImplementation((id) => ({
        select: jest.fn().mockReturnValue({
          lean: jest.fn().mockResolvedValue(mockModules.find(m => m._id.toString() === id.toString())),
        }),
      }));
    });
    ...
    it('should create default access control when none exists', async () => {
      // Arrange
      mockAccessControlRepository.findByProjectAndChannel.mockResolvedValue(null);
      ...
      // Assert
      expect(mockAccessControlRepository.findByProjectAndChannel).toHaveBeenCalledWith(
        projectId,
        channelId,
      );
      expect(mockAccessControlRepository.createOrUpdateAccessControl).toHaveBeenCalledWith(
        projectId,
        channelId,
        expectedModuleConfigs,
      );
    ...
    it('should throw NotFoundException when no active roles found', async () => {
      ...
      // Act & Assert
      await expect(
        accessControlService.getOrCreateDefaultAccessControl(projectId, channelId),
      ).rejects.toThrow(new NotFoundException('No active roles found for the channel'));
    });
    ...
    it('should throw NotFoundException when module not found during update', async () => {
      ...
      await expect(
        accessControlService.getOrCreateDefaultAccessControl(projectId, channelId),
      ).rejects.toThrow(new NotFoundException('Module with id 507f1f77bcf86cd799439099 not found'));
    });
  });
