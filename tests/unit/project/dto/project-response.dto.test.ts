...
function createMockModule(data: Partial<IModule> = {}): HydratedDocument<IModule> {
  const mockModule = {
...
function createMockProject(data: Partial<IProject> = {}): HydratedDocument<IProject> {
  const mockProject = {
    _id: data._id || new Types.ObjectId(),
...
describe('ProjectResponseDto', () => {
  ...
  it('should validate using class-validator decorators', async () => {
    ...
    if (module.moduleDetails) {
      ...
      expect(module.moduleDetails.permissions?.every(p => typeof p === 'string')).toBe(true);
    }
  });
}); 